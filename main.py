...
def get_args_parser():
    ...
    parser.add_argument('--log_dir', default='Experiment/Messidor/log',
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.main_eval == 'f1':
                ...
                temp = (test_stats["kappa"] + test_stats["spec"] + test_stats["sen"] + test_stats["auc"] + test_stats["accuracy_score"]) / 5
                ...
                if max_accuracy < temp:
                    ...
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch="best", model_ema=model_ema)
                print(f'Max average: {max_accuracy}, AUC: {test_stats["auc"]}, Kappa: {test_stats["kappa"]}, F1: {test_stats["f1"]}, npv: {test_stats["npv"]}, ppv: {test_stats["ppv"]}, SPEC: {test_stats["spec"]}, SEN: {test_stats["sen"]}')        
